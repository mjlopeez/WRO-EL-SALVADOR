          ...
          {filtered.map((t, i) => (
            <motion.div key={t.id} initial={{ opacity: 0, x: -16 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.03 }}
              ...
              <div className="flex-1 min-w-0">
                ...
                {(t.member1 || t.member2) && <p className="text-xs text-gray-600 truncate">{[t.member1, t.member2, t.member3].filter(Boolean).join(' · ')}</p>}
              </div>
              <span className={`text-xs px-2 py-0.5 rounded-full border shrink-0 hidden sm:inline ${catColors[t.category] || catColors.elementary}`}>
                {CATEGORY_META[t.category]?.label || t.category}
              </span>
              <JudgeSelector team={t} judges={judges} />
              <button onClick={() => openEdit(t)} className="text-gray-600 hover:text-green-400 transition-colors p-1"><Pencil size={15} /></button>
